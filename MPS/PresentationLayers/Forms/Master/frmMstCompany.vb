Public Class frmMstCompany
    ...
    Private Sub prvSetGrid()
        UI.usForm.SetGrid(grdView, "CompanyID", "ID", 100, UI.usDefGrid.gIntNum, False)
    ...
    Private Sub prvQuery()
        Try
            grdMain.DataSource = BL.Company.ListData
    ...
    Public Sub pubRefresh(Optional ByVal strSearch As String = "")
        With grdView
            If Not grdView.FocusedValue Is Nothing And strSearch = "" Then
                strSearch = grdView.GetDataRow(grdView.FocusedRowHandle).Item("CompanyID")
            End If
            prvQuery()
            If grdView.RowCount > 0 Then UI.usForm.GridMoveRow(grdView, "CompanyID", strSearch)
        End With
    End Sub
    ...
    Private Sub prvGet()
        ...
        If grdView.GetRowCellValue(intPos, "IDStatus") = VO.Status.Values.InActive Then
            UI.usForm.frmMessageBox("Tidak dapat pilih perusahaan " & grdView.GetRowCellValue(intPos, "Name") & ". Dikarenakan perusahaan tersebut sudah tidak aktif")
            Exit Sub
        Else
    ...
    Private Sub prvNew()
        Dim frmDetail As New frmMstCompanyDet
        With frmDetail
            .pubIsNew = True
    ...
    Private Sub prvDetail()
        ...
        With frmDetail
            .pubIsNew = False
            .pubID = grdView.GetRowCellValue(intPos, "CompanyID")
            .StartPosition = FormStartPosition.CenterScreen
            .pubShowDialog(Me)
    ...
    Private Sub prvDelete()
        ...
        Try
            BL.Company.DeleteData(grdView.GetRowCellValue(intPos, "CompanyID"))
            UI.usForm.frmMessageBox("Hapus data berhasil.")
            pubRefresh(grdView.GetRowCellValue(intPos, "CompanyID"))
        Catch ex As Exception
            UI.usForm.frmMessageBox(ex.Message)
        End Try
    End Sub
    ...
    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterCompany, VO.Access.Values.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterCompany, VO.Access.Values.DeleteAccess)
        End With
    End Sub

#Region "Form Handle"

    Private Sub frmMstCompany_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Close this form?") Then Me.Close()
        End If
    End Sub

    Private Sub frmMstCompany_Load(sender As Object, e As EventArgs) Handles MyBase.Load
