Public Class frmMstChartOfAccountGroup
    ...
    Private Sub prvSetGrid()
        ...
        UI.usForm.SetGrid(grdView, "AliasName", "Alias", 100, UI.usDefGrid.gString)
        UI.usForm.SetGrid(grdView, "Tipe", "Tipe", 100, UI.usDefGrid.gString)
    ...
    Private Sub prvQuery()
        Try
            grdMain.DataSource = BL.ChartOfAccountGroup.ListData()
    ...
    Private Sub prvGet()
        ...
        If grdView.GetRowCellValue(intPos, "IDStatus") = VO.Status.Values.InActive Then
            UI.usForm.frmMessageBox("Tidak dapat pilih group akun perkiraan " & grdView.GetRowCellValue(intPos, "Name") & ". Dikarenakan group akun perkiraan tersebut sudah tidak aktif")
            Exit Sub
        Else
    ...
    Private Sub prvNew()
        Dim frmDetail As New frmMstChartOfAccountGroupDet
        With frmDetail
            .pubIsNew = True
    ...
    Private Sub prvDelete()
        ...
        Try
            BL.ChartOfAccountGroup.DeleteData(grdView.GetRowCellValue(intPos, "ID"))
    ...
    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterChartOfAccountGroup, VO.Access.Values.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterChartOfAccountGroup, VO.Access.Values.DeleteAccess)
        End With
    End Sub

#Region "Form Handle"

    Private Sub frmMstChartOfAccountGroup_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Tutup form ini?") Then Me.Close()
        End If
    End Sub

    Private Sub frmMstChartOfAccountGroup_Load(sender As Object, e As EventArgs) Handles MyBase.Load
