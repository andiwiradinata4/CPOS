...
Public Class frmTraDownPaymentVer2
...
#End Region
    ...
    Private Sub prvSetGrid()
        ...
        UI.usForm.SetGrid(grdView, "DPDate", "Tanggal", 100, UI.usDefGrid.gSmallDate)
        UI.usForm.SetGrid(grdView, "PaymentReferencesID", "PaymentReferencesID", 100, UI.usDefGrid.gIntNum, False)
        UI.usForm.SetGrid(grdView, "PaymentReferencesName", "Pembayaran Melalui", 100, UI.usDefGrid.gString, False)
        UI.usForm.SetGrid(grdView, "ReferencesNote", "No. Referensi Pembayaran", 100, UI.usDefGrid.gString, False)
        UI.usForm.SetGrid(grdView, "TotalAmount", "Total Panjar", 100, UI.usDefGrid.gReal2Num)
        UI.usForm.SetGrid(grdView, "TotalUsage", "Panjar Terpakai", 100, UI.usDefGrid.gReal2Num)
    ...
    Private Sub prvFillCombo()
        Try
            Dim dtData As DataTable = BL.StatusModules.ListDataByModulesID(VO.Modules.Values.TransactionReceiveDownPayment)
            Dim dr As DataRow
            dr = dtData.NewRow
            With dr
                .BeginEdit()
    ...
    Private Sub prvQuery()
        ...
        pgMain.Value = 30
        Try
            grdMain.DataSource = BL.DownPayment.ListData(intCompanyID, MPSLib.UI.usUserApp.ProgramID, dtpDateFrom.Value.Date, dtpDateTo.Value.Date, cboStatus.SelectedValue, VO.DownPayment.Type.SalesService)
            prvSumGrid()
    ...
    Private Function prvGetData() As VO.DownPayment
        ...
        clsReturn.DPType = grdView.GetRowCellValue(intPos, "DPType")
        clsReturn.DPDate = grdView.GetRowCellValue(intPos, "DPDate")
    ...
    Private Sub prvNew()
        Dim frmDetail As New frmTraDownPaymentVer2Det
        With frmDetail
            .pubIsNew = True
            .pubCS = prvGetCS()
    ...
    Private Sub prvDelete()
        ...
        pgMain.Value = 40
        Try
            BL.DownPayment.DeleteData(clsData)
    ...
    Private Sub prvSumGrid()
        Dim SumTotalAmount As New GridColumnSummaryItem(DevExpress.Data.SummaryItemType.Sum, "TotalAmount", "Total Panjar: {0:#,##0.00}")
        Dim SumTotalUsage As New GridColumnSummaryItem(DevExpress.Data.SummaryItemType.Sum, "TotalUsage", "Total Terpakai: {0:#,##0.00}")

        If grdView.Columns("TotalAmount").SummaryText.Trim = "" Then
            grdView.Columns("TotalAmount").Summary.Add(SumTotalAmount)
        End If

        If grdView.Columns("TotalUsage").SummaryText.Trim = "" Then
            grdView.Columns("TotalUsage").Summary.Add(SumTotalUsage)
        End If
    End Sub

    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.TransactionReceiveDownPayment, VO.Access.Values.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.TransactionReceiveDownPayment, VO.Access.Values.DeleteAccess)
        End With
    End Sub

#Region "Form Handle"

    Private Sub frmTraDownPaymentVer2_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Tutup form?") Then Me.Close()
        End If
    End Sub

    Private Sub frmTraDownPaymentVer2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
